...
#rv = triangle.RotateXYZtoTop(v, up = 'uv')
#pltd = triangle.RotateXYZtoTop(txyz, up = 'uv')
#
...
def MakeAlphaColmap(Col):
    # Make a colormap with defined hue to allow values other than plain RGB for triangle (we'll want magenta sometimes)
...
def RotateXYZtoTop(v, up = 'uv'):
    # Rotate XYZ (an n by 3 numpy array) coordinates to look down (z) whatever axis is chosen
    # For example:

    # example triangle plot for TCS
    # v = np.array([[0, 0, .75], 
    #      [-.61237, -.35355, -.25], 
    #      [0, .70711, -.25], 
    #      [.61237, -.35355, -.25]]) 
...
def MakeTriangle(ax, v, vlab, fcolor = 'None', ecolor = 'red', ann = 1):
    # This will plot out triangle in color or without, with or without annotations, taking into account where colors should positionally.
    # fcolor is either 'None' or 'RGB', either white or colored in background
    # ecolor is edge color for the triangle
    # ann is annotation on or off
    ...
    # rv = RotateXYZtoTop(v, up = up)
    # pltd = RotateXYZtoTop(pltd, up = up)
    # MakeTriangle(ax, rv, vlab, fcolor = bgcolor, ecolor = 'None', ann = 0)
    ...
    # reorder those pverts and colors to go top, left, right - this is inelegant
