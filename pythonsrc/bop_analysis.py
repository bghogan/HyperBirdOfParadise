...
def RunDataAnalysis(lookin, force = True, mainplots = True, additionalplots = False, label_dict = None, color_dict = None, color_scale = 1):

    ## Function to run the data gathering process easily for each of the 3 sets of HSI we have
    # Normal (4 images per specimen)
    # Tilted (1 image per specimen, with bill tilted toward imager to better capture belly)
    # Tail (2 to 4 images per specimen, front and back of tail)

    # This runs across these 3 sets, and for each, samples out a bunch of spectra, as well as their udlr if exists
    # Then it embeds those spectra in various ways
    # Then it pickles (saves it to a file) for ease of use in the analysis

    # force = True # should we overwrite existing datafiles?
    ...
    for directory in directories: 
        ...
        utils.CheckFileMatch(datafiles["matfiles"], datafiles["hdrfiles"], datafiles["bilfiles"]) # check they're in the right order

        # load an associated mat file, to list the available masks
        image_example = 0
        ...
        desire = 1000 # how many spectra would we want per bird per patch? 1000 in main analysis
        ...
        if os.path.isfile(directory + '/Gatherdata_mask_0.pickle') and force == False: # Note: this checking fails if some but not all present
            ...
            for k in np.arange(len(mat_contents.bwlist))[np.sum(desirematrix, axis = 0)>0]:
                ProcessData(datafiles, k, desirematrix, directory, color_scale = color_scale)
    ...
    if mainplots: # Now automate generation of the figures for the paper here
        ...
        fig = bop_plotting.BasicPlot(cln, pltn, mat_contents, label_dict, color_dict, sort_by_rgb = False)
        fig.savefig('../Figures/' + pltn + '.png', dpi = 300, bbox_inches="tight")
        fig.savefig('../Figures/' + pltn + '.pdf', dpi = 300, bbox_inches="tight")
        ...
        fig = bop_plotting.BasicPlot(cln, pltn, mat_contents, label_dict, color_dict, lab = 'avb')
        fig.savefig('../Figures/CIELAB.png', dpi = 300, bbox_inches="tight")
        fig.savefig('../Figures/CIELAB.pdf', dpi = 300, bbox_inches="tight")

        # CieXYy plots
        includeWhole = 1 # include the first mask in the columns or exclude - as defined, this is whole mask
        # fig = CIEXYyPlot(cln, mat_contents, includeWhole, label_dict, color_dict)
        fig = bop_plotting.CIEXYyPlot_two(cln, mat_contents, includeWhole, label_dict, color_dict)
        fig.savefig('../Figures/CIEXYZ.png', dpi = 300, bbox_inches="tight")
        fig.savefig('../Figures/CIEXYZ.pdf', dpi = 300, bbox_inches="tight")

        # Tetracolor plot
        # %matplotlib inline
        fig = bop_plotting.tXYZplotNew_two('species', mat_contents, 1,  label_dict, color_dict, 'uv')
        fig.savefig('../Figures/tXYZ.png', dpi = 300, bbox_inches="tight")
        fig.savefig('../Figures/tXYZ.pdf', dpi = 300, bbox_inches="tight")
    ...
    if additionalplots:
        ...
        bop_plotting.SampleCheck_breast(owl, lookin) # may have been broken

        # plot all samples for all normal views
        directory = "../dat/Normal_hyperspectral_images"
        datafiles = LookUpData(open("../dat/Normal_hyperspectral_images.txt", "r"), lookin) 
        bop_plotting.SampleCheck_half(owl, pas = range(0,7), halfSpecimens = False, lookin = lookin) # will run on all patches
        
        # this plots the whole sample, but for only half the specimens - for main figure
        bop_plotting.SampleCheck_half(owl, pas = range(1), halfSpecimens = True, lookin = lookin)
        ...
        fig = bop_plotting.BasicPlot(cln, pltn, mat_contents, label_dict, color_dict, reembed = True, n_neighbors = 2000, min_dist = 0.1, sort_by_rgb = False)
        # fig.suptitle('n_neighbors:' + str(2000) + ', min_dist:' + str(0.1), fontsize=16)
        fig.savefig('../Figures/pcaUMAP_params_nn2000_md01.png', dpi = 300, bbox_inches="tight")
        fig.savefig('../Figures/pcaUMAP_params_nn2000_md01.png', dpi = 300, bbox_inches="tight")
        
        cln = 'species'
        pltn = 'None' # since reembedding, using only PCA (hardcoded)
        fig = bop_plotting.BasicPlot(cln, pltn, mat_contents, label_dict, color_dict, reembed = True, n_neighbors = 15, min_dist = 1, sort_by_rgb = False)
        # fig.suptitle('n_neighbors:' + str(15) + ', min_dist:' + str(1), fontsize=16)
        fig.savefig('../Figures/pcaUMAP_params_nn15_md1.png', dpi = 300, bbox_inches="tight")
        fig.savefig('../Figures/pcaUMAP_params_nn15_md1.png', dpi = 300, bbox_inches="tight")
...
def ProcessData(datafiles, k, desirematrix, directory, window_length = 45, color_scale = 1, saveout = True):
    ...
    # matlab, saved in .mat format). k indicates which index of which mask number from matfiles we're interested in
    ...
    sample_extent = 4
    arr, meta, owl = bop_sampling.SampleMask(datafiles, desirematrix, sample_extent, k, plotme = 'false') 
