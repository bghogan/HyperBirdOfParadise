...
# Loop through each mat file, work out how many samples we'll want to extract later for each specimen and patch. 
def FindMasks(datafiles, saveout):
    ...
    if os.path.isfile(matfilename):
        ...
        # we know beforehand what masks are what
        maskid = np.tile(np.arange(0,mat_contents.masklist.shape[0],1), [len(datafiles["matfiles"]), 1])

        # for each of them, open each of the masks and generate some sampling based on desire
...
def SampleMask(datafiles, desirematrix, n, k, plotme = 'false'):
    ...
    for i, j in zip(range(len(datafiles["matfiles"])), datafiles["matfiles"]):   
        ...
        # import udlr
        flr = "E:/OneDrive - Princeton University/BOP/Revamp 2023/Renders/MetaDepthNormal/" + utils.GetFilename(datafiles["bilfiles"][i]) + '_normal_R.tif'
        if os.path.isfile(flr):
            lr, ud, ma, de = bop_udlr.GetUDLR("E:/OneDrive - Princeton University/BOP/Revamp 2023/Renders/MetaDepthNormal/", datafiles["bilfiles"][i])
        ...
        if not np.isnan(mask).all():
            ...
            Out = Out / mat_contents.ref99[None,:] # use reference measure to correct to reflectance # CHECKME
            # this divisor might be in data terms not in 0-1 terms!
